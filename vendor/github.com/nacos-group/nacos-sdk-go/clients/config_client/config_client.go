...
import (
	...
	loginfo "github.com/kelseyhightower/confd/log"
	"io/ioutil"
	"log"
	"net/http"
	"os"
...
func (client *ConfigClient) GetConfig(param vo.ConfigParam) (content string, err error) {
	loginfo.Info("GetConfig is call " + param.NacosUsername)
...
func (client *ConfigClient) getConfigInner(param vo.ConfigParam) (content string, err error) {
	...
	cacheKey := utils.GetConfigCacheKey(param.DataId, param.Group, clientConfig.NamespaceId, param.NacosUsername, param.NacosPassword)
...
func (client *ConfigClient) PublishConfig(param vo.ConfigParam) (published bool,
	...
	if len(param.NacosUsername) <= 0 {
		err = errors.New("[client.PublishConfig] param.NacosUsername can not be empty")
	}
	if len(param.NacosPassword) <= 0 {
		err = errors.New("[client.PublishConfig] param.NacosPassword can not be empty")
	}
	loginfo.Debug("PublishConfig param is %v", param)
	clientConfig, _ := client.GetClientConfig()
...
func (client *ConfigClient) ListenConfig(param vo.ConfigParam) (err error) {
	loginfo.Debug("ListenConfig param is  %v", param)
	go func() {
		for {
...
func (client *ConfigClient) listenConfigTask(clientConfig constant.ClientConfig,
	...
	loginfo.Debug("listenConfigTask is param %v", param)
	// 检查&拼接监听参数
	client.mutex.Lock()
	if len(param.DataId) <= 0 {
	...
	loginfo.Debug("listeningConfigs is %s", listeningConfigs)
	client.mutex.Unlock()
	// http 请求
	params := make(map[string]string)
	params[constant.KEY_LISTEN_CONFIGS] = listeningConfigs
	params[constant.NACOS_USENAME] = param.NacosUsername
	params[constant.NACOS_PASSWORD] = param.NacosPassword
	var changed string
	// listen 请求http://10.2.156.184:8848/nacos/v1/cs/configs/listener 需要查看server端接收的参数
	...
	if strings.ToLower(strings.Trim(changed, " ")) == "" {
		loginfo.Debug("[client.ListenConfig] no change")
	} else {
		loginfo.Debug("[client.ListenConfig] config changed:" + changed)
		loginfo.Debug("updateLocalConfig param is %v", param)
...
func listen(agent http_agent.IHttpAgent, path string,
	...
	loginfo.Debug("[client.ListenConfig] request url:", path, " ;params:", params, " ;header:", header)
	...
	if err == nil {
		...
		if errRead != nil {
			...
			loginfo.Debug("response.StatusCode is %d", response.StatusCode)
			if response.StatusCode == 200 {
				changed = string(bytes)
				loginfo.Debug("response.Body bytes is %s", changed)
			} else {
...
func (client *ConfigClient) updateLocalConfig(changed string, param vo.ConfigParam) {
	client.mutex.Lock()
	defer client.mutex.Unlock()
	loginfo.Debug("changed values is %s", changed)
	...
	for _, config := range changedConfigs {
		...
		for i := 0; i < len(attrs); i++ {
			loginfo.Debug("attrs list is ", attrs[i])
		}
		...
		if len(attrs) == 2 {
			...
			if err != nil {
				...
				param.OnChange("", attrs[1], attrs[0], attrs[6], attrs[7], decrept)
			...
			content, err := client.getConfigInner(vo.ConfigParam{
				DataId:        attrs[0],
				Group:         attrs[1],
				NacosUsername: param.NacosUsername,
			...
			if err != nil {
				...
				// call listener:
				decrept, _ := client.decrypt(attrs[0], content)
				param.OnChange(attrs[2], attrs[1], attrs[0], param.NacosUsername, param.NacosPassword, decrept)
			}
		}
	}
